...
namespace daveos::core {
  ...
  // Subscriber view of a buffered record. Timestamp is captured at the log
  // call; message storage is valid only for the duration of the subscriber
  // callback. Names are borrowed from registration or platform context.
  // Outputters supply the line ending; callers need not include one in the
  // format string.
  struct LogRecord {
    Time timestamp;
    Level severity;
