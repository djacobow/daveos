...
#undef DAVEOS_STATUS_VALUES
  ...
  // even delay zero invokes the callback later in interrupt context, never
  // inline in arm(). quiesce() cancels and waits for callbacks, and must be
  // called outside an ISR. sequence()/notify() retain work notifications.
  // idle(deadline, sleep, observed) must avoid waiting if a notification
  // occurred after observed was sampled. The platform and any mutex it returns
  // must outlive all their users.
