...
namespace daveos::console {
  ...
  // Task-side write()/flush() collect a complete display/log frame, then copy
  // it atomically into the filling half of caller-owned storage (DMA-accessible
  ...
  // complete()/error() after hardware releases the buffer. P serializes task
  // and IRQ access; neither callback may run inline from start(). All
  // objects/storage must outlive outstanding transfers.
