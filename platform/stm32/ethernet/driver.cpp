...
namespace {
  ...
  net::Link ReadLink(void*) {
    ...
    if (next != net::Link::down && next != net::Link::fault) {
      // Reset descriptor ownership on reconnect, discarding stale queued
      // frames.
      HAL_ETH_DeInit(&eth);
      if (!InitMac()) {
        fault = true;
  ...
  bool Transmit(void*, std::span<const std::uint8_t> bytes) {
    ...
    for (auto& slot : dma.tx) {
      ...
      packet.pData = &slot;
      __DMB();
      if (HAL_ETH_Transmit_IT(&eth, &packet) == HAL_OK) {
        return true;
      }
      slot.used = false;
      return false;
...
extern "C" void HAL_ETH_ErrorCallback(ETH_HandleTypeDef*) {
  ...
  if (eth.DMAErrorCode & ETH_DMACSR_FBE) {
    fault = true;
  }
}
