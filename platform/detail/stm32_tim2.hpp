...
namespace daveos::platform::detail {
  ...
  class Stm32Tim2 : public core::Platform<Derived> {
   public:
    ...
    // Extended 64-bit microseconds since init. Rollover is serviced by the IRQ
    // or this read; interrupts must not stay masked for a full 32-bit counter
    // period.
    core::Time now();
    ...
    // inline. Large delays use intermediate compares. Requires successful
    // init().
    ...
    // ISR.
    void quiesce();
    // TIM2 IRQ entry point. Services rollover and invokes any due callback
    // once.
    void interrupt();
    ...
    void notify();
    // Check deadline/generation atomically before shallow WFI, or return to
    // poll when sleep is false. The scheduler must have armed its next wake
    // deadline.
