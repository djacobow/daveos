...
namespace daveos::platform::detail {
  ...
  core::Time Stm32Tim2<Derived>::now() {
    ...
    // Interrupts must not be masked for a full 32-bit period (about 71
    // minutes).
  ...
  void Stm32Tim2<Derived>::idle(core::Time deadline, bool sleep,
    ...
    core::Guard guard(*this);
    if (sequence_ != observed || now() >= deadline) {
      return;
    }
    if (sleep) {
    ...
    // Awake waiting returns to the scheduler to check queues and deadlines
    // again.
  }
}  // namespace daveos::platform::detail
