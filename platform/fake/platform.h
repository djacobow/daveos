...
namespace daveos::platform::fake {
  ...
  // the shared simulated interrupt domain. External host threads may also
  // inject interrupts.
  class Platform final : public detail::Synchronized<Platform> {
   public:
    ...
    explicit Platform(Advancement mode = Advancement::automatic)
        : mode_(mode) {}
    ...
    // Replace the pending timer. Expiry waits for time advancement, even at
    // delay 0.
    ...
    // Advance by microseconds and dispatch every timer due along the way.
    // Timers can rearm themselves. Inside an ISR only move time; defer nested
    // expiries until a surrounding or later non-ISR advancement can dispatch
    // them.
