...
namespace {
  ...
  constexpr core::Time kWatchdogTimeout = 5000000;
  ...
  h7::Watchdog watchdog;
  h7::Flash flash{&runtime,
                  [](void* p) { return static_cast<Runtime*>(p)->now(); },
                  [](void*) {
                    auto driver = watchdog.driver();
                    (void)driver.feed(driver.context);
                  }};
  daveos::util::crc32::Service crc{h7::crc32_backend()};
  ...
  void Initialize() {
    ...
    __enable_irq();
    RCC->AHB4ENR = RCC->AHB4ENR | RCC_AHB4ENR_GPIODEN;
  ...
  [[noreturn]] void Jump(std::uint32_t address) {
    ...
    USART3->CR1 = 0;
    // USART3's APB clock keeps D2 awake even while M4 waits in STOP. Release
    // the boot UART clock so the application's normal M4 handshake can run.
    RCC->APB1LENR = RCC->APB1LENR & ~RCC_APB1LENR_USART3EN;
    (void)RCC->APB1LENR;
    SCB->VTOR = address;
    __set_CONTROL(0);
...
extern "C" int main() {
  auto dog = watchdog.driver();
  if (dog.start(dog.context, kWatchdogTimeout) != core::Status::ok) {
    h7::reset();
  }
  Initialize();
  const auto& version = daveos::build::kBootloaderVersion;
  h7::set_fault_identity(version, 0);
  ...
  if (selection.status == core::Status::ok) {
    ...
    if (vectors[0] > 0x20000800 && vectors[0] <= 0x20020000 &&
      ...
      boot::Journal journal(flash.driver(), boot::kLayout);
      // Prepare checkpoint space while long erases cannot stall app tasks.
      if (journal.prepare_runtime(selection.slot) != core::Status::ok) {
        Log(core::Level::error, "Journal preparation failed; resetting");
        h7::reset();
      }
      (void)dog.feed(dog.context);
      Jump(address);
    }
