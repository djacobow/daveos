...
namespace daveos::net {
  ...
  struct TcpCallbacks {
    ...
    static err_t Receive(void* context, tcp_pcb*, pbuf* packet, err_t) {
      ...
      if (packet->tot_len > self.rx_.size() - self.received_) {
        return ERR_MEM;
      }
      const auto tail = (self.head_ + self.received_) % self.rx_.size();
      const auto first =
          std::min<std::size_t>(packet->tot_len, self.rx_.size() - tail);
      pbuf_copy_partial(packet, self.rx_.data() + tail,
                        static_cast<u16_t>(first), 0);
    ...
    static void Error(void* context, err_t) {
      static_cast<TcpServer*>(context)->Disconnect(
          false);  // PCB already freed.
    }
  };
  ...
  void TcpServer::consume(std::size_t size) {
    ...
    received_ -= size;
    if (client_ && size) {
      tcp_recved(client_, static_cast<u16_t>(size));
    }
  }
