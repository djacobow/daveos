...
namespace daveos::net {
  ...
  class TcpServer {
   public:
    ...
    // or another service/server call. consume() releases bytes and opens the
    // TCP window. Inspecting bytes alone does not acknowledge application
    // consumption.
    std::span<const char> peek() const;
    void consume(std::size_t size);
    // Copy all pieces atomically, or drop the complete record. Disconnected
    // output is not retained. Accepted data is delivered only while this
    // session survives.
