...
def main():
    ...
    sources = sorted(
        str(path) for directory in ("lib", "apps", "platform", "examples", "starters", "tests")
    ...
    if args.check.startswith("format"):
        ...
        command = [
            ...
            *[f"-i{folder}" for folder in GENERATED],
            f"--cppcheck-build-dir={cache}", "-I.", "-Ilib",
        ]
        subprocess.run(command + [name for name in
            ("lib", "apps", "platform", "examples", "starters")
        ...
        hal_headers = sorted(str(path) for path in (ROOT / "lib/hal").rglob("*")
                             if path.suffix in (".h", ".hpp"))
        hal_headers += [str(path) for path in (ROOT / "lib/drivers").rglob("*")
                        if path.suffix in (".h", ".hpp")]
