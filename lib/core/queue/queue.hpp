...
namespace daveos::core {
  ...
  // A query value is meaningful only when status is ok (or bool(result) is
  // true).
  template <typename T>
  struct Result {
    Status status;
  ...
  // Mutex acquisition is a single try_lock(): busy leaves queue/output
  // unchanged. State queries can also return busy, so their value must not be
  // used on failure. The platform must outlive the queue. T has the same
  // constraints as Queue.
